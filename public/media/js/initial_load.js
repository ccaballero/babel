...
var height=function(){if(window.innerHeight){return window.innerHeight;}else{return document.documentElement.clientHeight;}};
var width=function(){if(window.innerWidth){return window.innerWidth;}else{return document.documentElement.clientWidth;}};
var resize=function(){$('#wrapper').css('height',height()+'px');$('#main').css('height',(height()-60)+'px');$('#wrapper').css('width',width()+'px');$('#main').css('width',(width()-20)+'px');};

$(window).resize(resize);
$(document).ready(resize);

$(document).ready(function(){
$('#menubar a.main').hover(function(){$(this).animate({top:'1.0em'},100);},function(){$(this).animate({top:'0.5em'},100);});
$.get('/images.json',function(data){var json=eval('('+data+')');var images=json.images;var image=images[Math.floor(Math.random()*images.length)];$('img.switcher').attr('src',image).animate({opacity: 1.0},3500);});
...
$('.update_file').overlay({left:'center',top:90,onBeforeLoad:function(){$.get('/books/'+book.substring(5)+'/info.json',function(data){var json=eval('('+data+')');$('#bookstore option').each(function() {if($(this).attr('label')==json.book.bookstore){$(this).attr('selected','selected');}else{$(this).removeAttr('selected');}});$('#bookstore').focus();$('#directory').attr('value',json.book.directory);$('#file').attr('value',json.book.file);$('#form_file').attr('action','/books/collection/'+book.substring(5)+'/edit');});}});
...
$('div.book').overlay({
    ...
    onBeforeLoad:function(){
        $.get('/books/'+book.substring(5)+'/info.json',function(data){
            var json=eval('('+data+')');
